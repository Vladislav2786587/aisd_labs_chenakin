...
class MicrowaveApp:
    ...
    def select_food(self, food_name):
        if not self.microwave.is_door_open:   #запрещает выбор продукта при закрытой дверце
            return
    ...
    def draw_microwave(self):
        ...
        self.canvas.create_text(
            ...
            text="",  # сам текст обновляется в update_display_text
            fill="#4CFF96",
    ...
    def draw_food_inside(self):
        ...
        if food_name in self.food_images:
            ...
            display_img = self.rotate_image(food_img, self.rotation_angle_y)
    ...
    def create_buttons(self):
        ...
        self.btn_start = tk.Button(
            self.root, text="СТАРТ\nПАУЗА", width=10, height=2,
            bg="#33aa33", fg="white", activebackground="#44cc44",
            font=btn_font, bd=0, relief="flat",
            command=self.start
        )
        self.btn_stop = tk.Button(
            self.root, text="СБРОС", width=10, height=2,
    ...
    def rotate_image(self, img, angle):
        ...
        k = max(0.1, abs(math.cos(math.radians(a))))
        w, h = img.size
        new_w = max(1, int(w * k))
